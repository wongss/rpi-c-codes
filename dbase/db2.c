...
void readNExec( void ) {
	int rc;
	while(1) {
		printf("\nEnter your SQL statement with ';' behind (\".exit\" to quit):");
		gets(queryStr);
...
int main ( int argc, char **argv ) {
	...
	if ((rc = showTableNames(db)) < 0) {
		printf("\nShow table name error"); fflush(stdout);
		goto quit;
	}
	if (rc == 0) {
	...
	if (showTableInfo(db, tableName) < 0) {
		printf("\nShow table info error"); fflush(stdout);
		goto quit;
	}
	...
	readNExec();
	printf("\nQuery and processing done by now"); fflush(stdout);
	printf("\n");
quit:
